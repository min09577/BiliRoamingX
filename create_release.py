...
data = json.dumps({"tag_name": "v1.72.0", "name": "v1.72.0", "body": "v1.72.0 - 动态自动点赞、弹幕翻译、评论默认排序、双击手势自定义", "draft": False, "prerelease": False}).encode()
...
try:
    ...
    apk = "integrations/app/build/outputs/apk/release/BiliRoamingX-integrations-1.72.0.apk"
    with open(apk, "rb") as f: apk_data = f.read()
    ureq = urllib.request.Request(upload_url + "?name=BiliRoamingX-integrations-1.72.0.apk", data=apk_data, method="POST")
